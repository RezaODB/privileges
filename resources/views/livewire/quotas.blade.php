<div class="relative">
    <form wire:submit class="grid grid-cols-[auto_1fr_auto]">
        ...
        <div class="sm:border-l-2 border-zinc-800 pt-6 px-4 flex gap-4">
            <input type="radio" wire:model.live="answers.{{ $item->id }}" value="yes" class="rounded-none bg-transparent w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0"> 
            <input type="radio" wire:model.live="answers.{{ $item->id }}" value="no" class="rounded-none bg-transparent w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0"> 
        </div>
        @endforeach
    </form>
</div>
