<form wire:submit class="grid grid-cols-1 lg:grid-cols-3 gap-16 px-2 sm:px-8 mt-16">
    <div class="grid grid-cols-[1fr_auto] items-center w-full lg:col-span-2">
        ...
        <div class="py-3 px-4 flex gap-4 items-center">
            ...
            <div class="group" wire:click="$set('votes.{{ $item->id }}', '{{ rand(0, 1) === 1 ? 'yes' : 'no' }}')">
                <div class="w-6 h-6 border border-zinc-800 rounded-full bg-white hover:bg-zinc-800 duration-300 cursor-pointer group-hover:[transform:rotateX(180deg)]"></div>
            </div>
        </div>
        @endforeach
    </div>
