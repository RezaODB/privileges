<section class="px-2 sm:px-8 mt-16">
    ...
    <form wire:submit>
              ...
              <table class="relative w-full">
                ...
                <tbody class="divide-y divide-zinc-800">
                    ...
                    <tr class="py-3">
                        ...
                        <td class="px-2"><input type="radio" wire:model.live="answers.{{ $item->id }}" value="always" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"></td>
                        <td class="px-2"><input type="radio" wire:model.live="answers.{{ $item->id }}" value="often" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"></td> 
                        <td class="px-2"><input type="radio" wire:model.live="answers.{{ $item->id }}" value="sometimes" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"></td> 
                        <td class="px-2"><input type="radio" wire:model.live="answers.{{ $item->id }}" value="rarely" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"></td> 
                        <td class="px-2"><input type="radio" wire:model.live="answers.{{ $item->id }}" value="never" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"></td> 
                        <td class="px-2"><input type="checkbox" wire:model.live="boosters" value="{{ $item->id }}" class="rounded-full w-6 h-6 mx-2 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent" {{ count($boosters) >= 3 && !in_array($item->id, $boosters) ? 'disabled' : '' }}> 
                    </tr>
                    @endforeach
                </tbody>
              </table>
        ...
        <div class="py-3 border-b-2 border-zinc-800 border-dotted flex items-center justify-end gap-4 ml-px">
            <input type="radio" wire:model.live="answers.{{ $item->id }}" value="always" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none"> 
        ...
        <div class="flex flex-col lg:flex-row gap-x-6 gap-y-4 mt-12">
            ...
            <textarea wire:model.live.debounce.1000ms="answers.comment" id="comment" rows="5" class="w-full h-full focus:ring-0 disabled:bg-transparent"></textarea>
        </div>
