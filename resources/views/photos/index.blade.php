        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 space-y-4">
                    <a href="{{ route('photos.create') }}" class="px-4 py-2 bg-green-600 rounded-md text-white inline-block">Create</a>
                    <div class="divide-y">
                        @foreach ($photos as $item)
                            <div class="flex gap-4 items-center p-1 hover:bg-gray-100">
                                <form action="{{ route('photos.update', $item) }}" method="post">
                                    @csrf
                                    @method('patch')
                                ...
                                <a href="{{ route('photos.edit', $item) }}" class="text-blue-600 hover:underline">{{ $item->title }}</a>
                                <form action="{{ route('photos.destroy', $item) }}" method="post" class="ml-auto">
                                    @csrf
                                    @method('delete')
