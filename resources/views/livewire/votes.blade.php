<form wire:submit class="grid grid-cols-[1fr_auto] px-2 sm:px-8 mt-16">
    ...
    <div class="py-3 border-b-2 border-zinc-800 border-dotted flex items-center justify-end gap-4 ml-px">
        <input type="radio" wire:model.live="votes.{{ $item->id }}" value="yes" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none" disabled> 
        <input type="radio" wire:model.live="votes.{{ $item->id }}" value="no" class="rounded-none w-8 h-3 text-zinc-800 checked:bg-none focus:ring-0 disabled:bg-transparent disabled:border-none" disabled>
        <div class="group hover:scale-150 duration-700 hidden" wire:click="$set('votes.{{ $item->id }}', '{{ rand(0, 1) === 1 ? 'yes' : 'no' }}')">
    ...
    <div class="flex flex-col lg:flex-row gap-x-6 gap-y-4 mt-12 col-span-2">
        ...
        <textarea wire:model.live.debounce.1000ms="votes.comment" id="comment" rows="5" class="w-full h-full focus:ring-0 disabled:bg-transparent" disabled></textarea>
    </div>
</form>