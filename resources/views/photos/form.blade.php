<div class="grid grid-cols-1 gap-4">
    ...
    <input type="text" name="title" placeholder="Title" value="{{ old('title') ?? $photo->title }}" class="border-gray-200 shadow rounded-md" required>
    @error('title')<div class="text-red-500">{{ $message }}</div>@enderror

    <textarea name="body" class="editor" placeholder="Body">{{ old('body') ?? $photo->body }}</textarea>
