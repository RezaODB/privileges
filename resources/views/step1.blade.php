...
<section class="bg-white py-16 shadow-xl relative">
    ...
    <div class="cursor-pointer relative" x-data="{ open: false }" x-on:click="open = !open">
        <div class="flex items-center justify-between gap-4 mx-4 md:mx-8 border-black {{ $loop->first ? '' : 'border-t pt-4' }}">
        ...
        <div class="mt-12 overflow-hidden relative pb-32" x-bind:class="open ? 'h-full' : 'h-0'">
            <div class="grid grid-cols-1 md:grid-cols-3 pl-4 md:pl-0 gap-16">
                <div class="order-2 md:order-1 flex flex-col justify-around gap-8">{!! $item->quotes !!}</div>
