<div class="grid grid-cols-1 gap-4">
    ...
    <select name="lang" class="justify-self-start border-gray-200 shadow rounded-md">
        <option value="fr" {{ old('lang', $sculpture->lang) === 'fr' ? 'selected' : '' }}>FR</option>
        <option value="en" {{ old('lang', $sculpture->lang) === 'en' ? 'selected' : '' }}>EN</option>
    </select>
    @error('lang')<div class="text-red-500">{{ $message }}</div>@enderror

    <input type="text" name="title" placeholder="Title" value="{{ old('title') ?? $sculpture->title }}" class="border-gray-200 shadow rounded-md" required>
    @error('title')<div class="text-red-500">{{ $message }}</div>@enderror

    <textarea name="body" class="editor" placeholder="Body">{{ old('body') ?? $sculpture->body }}</textarea>
