...
<section class="py-12 sm:px-4 grid grid-cols-[auto_1fr_auto]">
    ...
    <div class="border-l-4 border-black pt-6 px-4 flex gap-4">
        <input type="radio" name="question{{ $item->id }}" value="yes" class="rounded-none border-none w-8 h-3 text-black checked:bg-none focus:ring-0"> 
        <input type="radio" name="question{{ $item->id }}" value="no" class="rounded-none border-none w-8 h-3 text-black checked:bg-none focus:ring-0"> 
    </div>
    @endforeach
</section>
